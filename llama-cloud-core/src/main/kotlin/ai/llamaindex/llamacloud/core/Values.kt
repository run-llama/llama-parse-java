...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { LlamaCloudInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw LlamaCloudInvalidDataException("`$name` is not set")
            is JsonNull -> throw LlamaCloudInvalidDataException("`$name` is null")
            else -> throw LlamaCloudInvalidDataException("`$name` is invalid, received $this")
        }
