...
interface SplitServiceAsync {
     ...
     * `1-50`, `1,3,5-7`). Pages are read in ascending document order, and each segment's `pages`
     * are the parse job's own page numbers, so segments map straight back to the original document.
     * Requires a parse job as `file_input`; passing it with a file ID returns 400.
     *
     * ## Parse settings
     *
     * `configuration.parse_tier` and `configuration.parse_config_id` control how the document is
     * read before splitting; both are ignored when a parse job is supplied. A parse configuration
     * restricted to a page subset (`target_pages` or `max_pages`) is rejected, since split results
     * always number pages relative to the full document.
     *
