 ...
 * and/or `webhook_configuration_ids` referencing saved configurations. Batches emit `batch.pending`
 * on create, `batch.running` once processing starts, and a terminal `batch.success` or
 * `batch.error`.
 *
 * `batch.success` means the batch finished mapping every source file to a job — individual files
 * may still have failed, so read `results` (with `expand=results`) for per-file outcomes.
 *
 * Delivery order across events is not guaranteed; key on the `status` field in the payload rather
 * than arrival order.
 */
...
private constructor(
    ...
    private constructor(
        ...
        override fun toString() =
            "Body{config=$config, sourceDirectoryId=$sourceDirectoryId, webhookConfigurationIds=$webhookConfigurationIds, webhookConfigurations=$webhookConfigurations, additionalProperties=$additionalProperties}"
    }
