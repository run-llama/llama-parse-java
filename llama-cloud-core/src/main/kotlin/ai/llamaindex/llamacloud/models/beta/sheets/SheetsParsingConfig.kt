...
private constructor(
    ...
    /**
     * Deprecated: controlled by `tier`. Influences how likely similar-looking regions are merged
     * into a single table. Honored only on `agentic`.
     *
     * @throws LlamaCloudInvalidDataException if the JSON field has an unexpected type (e.g. if the
     *   server responded with an unexpected value).
     */
    ...
    /**
     * Spreadsheet extraction tier. `cost_effective` uses the rule-based/ML-only pipeline; `agentic`
